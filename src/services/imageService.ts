...
const hasImage = async (product: any): Promise<boolean> => {
  ...
  try {
    ...
    const response = await fetch(`https://lyme-back.vercel.app/api/producto/${product._id}/imagen`, {
      headers: {
  ...
  try {
    ...
    for (let i = 0; i < uncachedIds.length; i += batchSize) {
      ...
      const checks = await Promise.all(batch.map(async id => {
        try {
          const response = await fetch(`https://lyme-back.vercel.app/api/producto/${id}/imagen`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
...
const getImageUrl = (productId: string, width = 80, height = 80, quality = 80): string => {
  return `https://lyme-back.vercel.app/api/producto/${productId}/imagen?quality=${quality}&width=${width}&height=${height}&_=${new Date().getTime()}`;
};
