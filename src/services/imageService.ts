...
const hasImage = async (product: any): Promise<boolean> => {
  ...
  try {
    ...
    const response = await fetch(`http://179.43.118.101:4000/api/producto/${product._id}/imagen`, {
      headers: {
  ...
  try {
    ...
    for (let i = 0; i < uncachedIds.length; i += batchSize) {
      ...
      const checks = await Promise.all(batch.map(async id => {
        try {
          const response = await fetch(`http://179.43.118.101:4000/api/producto/${id}/imagen`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
...
const uploadImageBase64 = async (productId: string, base64Data: string): Promise<any> => {
  ...
  const response = await fetch(`http://179.43.118.101:4000/api/producto/${productId}/imagen-base64`, {
    method: 'POST',
    headers: {
...
const deleteImage = async (productId: string): Promise<any> => {
  ...
  const response = await fetch(`http://179.43.118.101:4000/api/producto/${productId}/imagen`, {
    method: 'DELETE',
    headers: {
      'Authorization': `Bearer ${token}`,
    },
...
const getImageUrl = (productId: string, width = 80, height = 80, quality = 80): string => {
  return `http://179.43.118.101:4000/api/producto/${productId}/imagen?quality=${quality}&width=${width}&height=${height}&_=${new Date().getTime()}`;
};
